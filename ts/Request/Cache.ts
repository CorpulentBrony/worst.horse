...
function clearCache(): void { File.unlinkDirFiles(DIR).catch(console.error); }
...
export async function set(url: string, headers: CacheHeaders): Promise<boolean> {
	const hash: string = Key.from(url).hash;
	const created: number = Date.now();
	const etag: string = headers.etag ? headers.etag : "";
	const mimeType: string = headers["content-type"] ? headers["content-type"]! : DEFAULT_MIME_TYPE;
	...
	if (expires === undefined)
		if (etag === "")
			...
			expires = created;
	return REDIS.setObject<Entry.Object>(hash, { created, etag, expires, mimeType, size });
}
